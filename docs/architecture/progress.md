...
项目已经完成 Stage 2 Core 资源写入和 MySQL/PostgreSQL 真实验证、Stage 3 插件包校验与状态持久化、Stage 4 插件独立进程与网关基础能力、Stage 5 OpenAPI 客户端和合同聚合、Stage 6 前端插件运行时基础能力，并完成 Stage 7 第一版插件生命周期 API 和管理页面。主业务仍采用编译进主应用的模块化方式。
...
- Stage 7 已加入持久化插件操作记录、生命周期状态机和唯一操作 ID，支持安装、启用、停用、升级、卸载以及升级失败恢复旧版本。
- Stage 7 已加入插件版本、生命周期日志、操作详情和卸载确认 API；生命周期 API 已进入 Core Admin OpenAPI 并生成 TypeScript 客户端，上传使用 multipart `FormData`。
- Stage 7 已加入 Core 插件 Resource 和生命周期管理页，复用 Resource Engine，支持 ZIP 安装、版本升级、启用、停用和默认保留数据的卸载。
- Stage 7 已加入插件生命周期审计写入和 MySQL/PostgreSQL 兼容的 `plugin_operations` 迁移。
- Stage 7 已加入当前进程生命周期互斥、启动时 enabled 插件进程恢复、精确插件详情接口和 `plugins.manage` 角色权限定义；安装不会覆盖正在运行的当前版本，卸载删除数据前会先停止插件。
...
- Stage 4：运行时进程/网关基础能力已完成；运行时 registry 仍是进程监督的易失状态，由 Stage 7 启动恢复逻辑根据数据库状态重建。
...
- Stage 7：插件迁移/权限/菜单的事务化注册、异步操作队列与进度轮询、完整失败矩阵和浏览器生命周期 E2E 仍待完成。
- Stage 8–9：插件静态资产发布、安全加固和发布流程。
